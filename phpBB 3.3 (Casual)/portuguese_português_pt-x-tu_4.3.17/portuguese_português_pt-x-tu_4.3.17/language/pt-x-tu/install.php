...
$lang = array_merge($lang, [
	...
	'PCRE_UTF_SUPPORT_EXPLAIN'				=> 'O phpBB não será executado se a tua instalação PHP não for compilada com suporte UTF-8 na extensão PCRE.',
	...
	'PHP_SUPPORTED_DB_EXPLAIN'				=> 'Deves ter suporte para pelo menos uma base de dados compatível dentro do PHP. Se nenhum módulo da base de dados for mostrado como disponível, deves entrar em contacto com o teu provedor de hospedagem ou rever a documentação de instalação relevante do PHP para obteres orientação.',
...
$lang = array_merge($lang, [
	...
	'TIMEOUT_DETECTED_MESSAGE'	=> 'O instalador detetou um tempo limite, podes tentar atualizar a página, o que pode levar a corrupção de dados. Sugerimos que aumentes as tuas configurações de tempo limite ou tentes usar o CLI.',
]);
...
$lang = array_merge($lang, [
	...
	'DB_HOST_EXPLAIN'			=> 'DSN significa Data Source Name e apenas é relevante para instalações ODBC. No PostgreSQL, usa localhost para ligar ao servidor local através do domínio socket UNIX e 127.0.0.1 para conectares através do TCP. Para SQLite, insere o caminho completo para o ficheiro da tua Base de Dados.',
...
'UPDATE_INSTALLATION_EXPLAIN'	=> 'Com esta opção, é possível atualizar a instalação do phpBB para a última versão.<br />Durante o processo, todos os ficheiros serão verificados. Podes rever todas as diferenças e ficheiros antes da atualização.<br /><br />A própria atualização do ficheiro pode ser realizada de duas maneiras.</p><h2>atualização Manual</h2><p>Com esta atualização, apenas transferes a tua seleção pessoal dos ficheiros modificados, e para teres a certeza de que não perderás as modificações do teu ficheiro, deves finalizar. Depois de teres transferido este pacote, deves manualmente enviar os ficheiros para as suas respetivas pastas. Em seguida, podes fazer a verificação dos ficheiros novamente para ver se os moveste corretamente.</p><h2>atualização automática com FTP</h2><p>Este método é similar ao primeiro, mas não precisas de descarregar os ficheiros modificados e enviá-los para o servidor. Isto será executado de forma automática. Para utilizares este método, tens que saber os dados da tua conta no servidor de FTP se isso for pedido. Depois de pronto, serás redirecionado para nova verificação dos ficheiros para teres a certeza de que tudo foi atualizado corretamente.<br /><br />',
	'UPDATE_INSTRUCTIONS'			=> '

		<h1>Anúncio de lançamento</h1>

<p>Por Favor, lê o Anúncio do Lançamento Oficial da Nova Versão antes de continuares o processo de atualização, poderás encontrar informações importantes. Também encontrarás disponíveis os atalhos para transferência do pacote, bem como o registo das alterações.</p>

		<br />

<h1>Como atualizar a tua instalação com o Pacote Completo (Full Package)</h1>

<p>O método recomendado de atualizares a tua instalação é via pacote completo. Caso os ficheiros originais do phpBB tenham sido modificados na tua instalação, poderás desejar usar o pacote de atualização avançada para evitar perder essas alterações. Também podes atualizar a tua instalação por outros métodos listados no INSTALL.html. As etapas para atualizares o phpBB3 via pacote completo são as seguintes:</p>
...
<li><strong><a href="%1$s" title="%1$s">Agora inicia o processo de atualização, apontando o teu navegador para a pasta de instalação</a>.</strong></li>
<li>Segue as etapas para atualizares a base de dados e deixa que seja executado até a conclusão.</li>
...
<p>Agora tens um novo fórum atualizado que contém todos os teus utilizadores e mensagens. Tarefas complementares:</p>
		<ul style="margin-left: 20px; font-size: 1.1em;">
<li>Atualiza o teu pacote de idioma</li>
<li>Atualiza o teu estilo<br /><br /></li>
		</ul>

<h1>Como atualizar a tua instalação com o Pacote de Atualização Avançada</h1>
...
<strong><a href="%1$s" title="%1$s">Agora inicia o Processo de atualização inserindo no teu navegador o endereço da pasta /install</a>.</strong><br />
		<br />
Em seguida serás conduzido ao longo do processo de atualização. Serás avisado quando a atualização estiver concluída.
		</p>
	',
]);
...
$lang = array_merge($lang, [
	...
	'UPDATE_FILE_METHOD_TITLE'			=> 'Métodos de atualização de ficheiros',

	'UPDATE_FILE_METHOD'					=> 'Método do atualização de ficheiro',
...
'NO_UPDATE_FILES_UP_TO_DATE'	=> 'A tua versão está atualizada. Não é necessário executar a ferramenta de atualização. Se desejas ter a certeza absoluta, verifica se enviaste corretamente os Ficheiros de atualização.',
	...
	'INCOMPATIBLE_UPDATE_FILES'	=> 'Os ficheiros de atualização encontrados são incompatíveis com a versão do phpBB instalada. A tua versão do phpBB é da %1$s e o Ficheiro de atualização é da versão %2$s para %3$s.',
]);
...
$lang = array_merge($lang, [
	...
	// File status
'UPDATE_CONTINUE_FILE_UPDATE'		=> 'Atualiza ficheiros',
...
'FILES_NEW_EXPLAIN'				=> 'Os ficheiros seguintes não existem atualmente na tua instalação.',
	...
	'UPDATE_FILE_UPDATER_HAS_FAILED'		=> 'A atualização de ficheiros “%1$s“ falhou. O instalador irá tentar com “%2$s“.',
	'UPDATE_FILE_UPDATERS_HAVE_FAILED'	=> 'A atualização de ficheiros falhou. Não existe métodos alternativos disponíveis.',
...
'CONTINUE_CONVERT_BODY'		=> 'Foi detetada uma tentativa prévia de conversão. Deves escolher entre iniciar uma nova conversão ou continuar a conversão interrompida.',
...
'REFRESH_PAGE'					=> 'Atualiza a página para continuares a conversão',
'REFRESH_PAGE_EXPLAIN'		=> 'Se clicares em "Sim", o conversor irá atualizar a página para continuar a conversão até à etapa final. Se esta é uma conversão apenas de teste e para determinação de erros, clica em "Não".',
...
'CONVERT_COMPLETE_EXPLAIN'	=> 'O teu Fórum foi convertido com sucesso para o phpBB 3.3. Já podes entrar e <a href="../">aceder ao Fórum</a>. Por favor, verifica se as configurações foram transferidas corretamente antes de ativares o teu fórum. Apaga a pasta de instalação. Lembra-te que está disponível ajuda online através da <a href="https://www.phpbb.com/support/docs/en/3.3/ug/">Documentação</a> e de <a href="https://www.phpbb.com/community/viewforum.php?f=661">fóruns de suporte</a>.',
...
'COLLIDING_USERNAMES_FOUND'		=> 'Nomes de utilizador duplicados foram encontrados no teu antigo fórum. Para concluíres a conversão, exclui ou renomeia esses utilizadores para que haja apenas um utilizador no teu fórum antigo para cada nome de utilizador limpo.',
	'CONV_ERR_FATAL'						=> 'Erro fatal de conversão',
'CONV_ERROR_ATTACH_FTP_DIR'		=> 'O envio de anexos por FTP está ativo no teu painel antigo. Desativa a opção de envio por FTP, cria uma pasta válida e envia todos os ficheiros de anexos para a nova pasta. Após concluíres esta operação, reinicia o conversor.',
...
'CONV_OPTIONS_BODY'					=> 'Esta página precisa do acesso ao código do Fórum. Introduz os dados da Base de Dados; o conversor não irá mexer no código atual. O acesso ao código deve ser desativado para evitar erros.',
	'CONV_SAVED_MESSAGES'				=> 'Mensagens Guardadas',

'PRE_CONVERT_COMPLETE'				=> 'Todos os passos da pré-conversão foram terminados com sucesso. Podes iniciar o processo de conversão. Por Favor, nota que deves ajustar algumas coisas manualmente. Depois da conversão, verifica as permissões atribuídas, reconstrói o índice de Pesquisa se necessário e verifica se os ficheiros foram copiados corretamente, como por exemplo os Avatares e as Emoções.',
]);
